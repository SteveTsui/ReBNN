...
from rebnn import rebnn18
import torchvision.models as models

parser = argparse.ArgumentParser("rebnn18")
...
def main():
    ...
    for pname, p in model_student.named_parameters():
        if 'binary_conv.u' in pname:
           recurrent_parameters.append(p)
        elif p.ndimension() == 4 or 'conv' in pname:
    ...
    optimizer = torch.optim.Adam(
        ...
        {'params' : recurrent_parameters, 'lr': 0.1 * args.learning_rate}],)
    ...
    if os.path.exists(checkpoint_tar):
        ...
        model_student.load_state_dict(checkpoint['state_dict'], strict=True)
